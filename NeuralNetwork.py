...
class FeedForwardNeuralNetwork:
  ...
  def modelFittingForBestConfig(self,beta,beta1,beta2,eps,optimizer,batch_size):
    '''
      ...
      Returns :
        None
      Function:
        calls particular optimizer function based on the algorithm called. this function is called to report the weights and biases of the best configuration (question 7)
    '''
    w,b=dict(),dict()
  ...
  def modelFittingForCeVsMse(self,beta,beta1,beta2,eps,optimizer,batch_size):
    '''
      Parameters:
        batch_size
        beta, beta 1, beta 2 : parameters used for updation
      ...
      Returns :
        None
      Function:
        calls particular optimizer function based on the algorithm called. this function is called to log the plots of cross entropy and mse loss(question 8)
    '''
    '''generates run name and logs it to wandb'''
    ...
    if(optimizer=="sgd"):
      self.stochastic_gradient_descent(batch_size)
    elif(optimizer=="momentum"):
      self.momentum_gradient_descent(beta,batch_size)
    elif(optimizer=="nag"):
      self.nestrov_gradient_descent(beta,batch_size)
    elif(optimizer=="rmsprop"):
      self.rmsprop(beta,eps,batch_size)
    elif(optimizer=="adam"):
      self.adam(beta1,beta2,eps,batch_size)
    elif(optimizer=="nadam"):
      self.nadam(beta1,beta2,eps,batch_size)